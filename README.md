...
1. 用浏览器打开 `dist/install.html`（双击文件即可）。
2. 把页面里那个红色「中文卡图」按钮**拖到书签栏**
   （多数浏览器里 <kbd>⌥⌘B</kbd> 或 <kbd>Ctrl+Shift+B</kbd> 显示书签栏；
   拖不动就右键它 → 「添加到书签」）。
3. 打开 <https://phase-rs.dev>，进入牌桌或牌组，**点一下书签栏里的「中文卡图」**。

> **每次重新加载页面后要再点一下。** 页面内的跳转（换牌桌、翻牌组、开关弹窗）不需要
> ——脚本一旦注入就管到本次加载结束。这就是「一个书签、零安装」的代价。

安装页里还留了一行可直接复制的书签地址，拖拽和右键都不灵时用它。

**换浏览器或换设备不用重新装**：在新浏览器里新建一个书签，把它的**地址**整段替换成
`dist/phase-rs-zh.bookmarklet.txt` 里的那一行即可。任何浏览器、任何系统都行
（Safari / Chrome / Edge / Firefox / Brave⋯⋯，macOS / Windows / Linux / iOS / Android）。

它**不检查页面域名**，只认 `cards.scryfall.io` 的卡图 URL——所以自建实例、部署在其他
域名的 phase.rs 也一样生效。
...
├── tools/
│   ├── build-bookmarklet.mjs               从 src/content.js 生成 dist/ 三件套
